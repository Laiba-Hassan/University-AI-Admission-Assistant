...
--   app_queue    : LOGIN, no BYPASSRLS, no access to tenant tables. Owns only pg-boss's own "pgboss" schema (job plumbing);
--                  it needs CREATE on the database because pg-boss installs its tables on startup.
...
CREATE ROLE app_queue    LOGIN PASSWORD 'queue_dev'    NOSUPERUSER NOBYPASSRLS;
CREATE ROLE app_resolver NOLOGIN BYPASSRLS;
GRANT CREATE ON DATABASE admission TO app_queue;
